...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  if ( output_.writer().is_closed()
       || ( first_index >= first_unassembled_index + output_.writer().available_capacity() )
       || ( first_index >= eof_index ) ) {
    return;
  }
  ...
  // 然后还有一个问题，就是reassembler的internal
  // storage在合并之后只能存不大于output的avaliablecapacity的数据，注意是合并后 即前后都要截取一次
